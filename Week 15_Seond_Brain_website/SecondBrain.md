...
**We have made a custom hook known as `useContent` hook to get the continuous data from the backend**
...
export function useContent() {
    ...
    useEffect(() => { // after every 10 second, it should see that whether new content has been added or not if yes then store it in the database and then show it
        refresh();
        let interval = setInterval(() => {
            refresh();
        }, 10 * 1000);

        return () => clearInterval(interval); // Cleanup is done to optimise the process 
    }, []);
                ...
                <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-lg relative">
                    ...
                    >
                        ...
                        <div className="relative">
                            ...
                            >
                                {showPassword ? ( // Added an icon for showing or hiding the password
...
**Similar to this will be `Signin.tsx`**
...
export const Signin = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // On successful signup, retrieve the token given from the backend
            const token = response.data.token;
            if (!token) throw new Error("Token missing from response");

            localStorage.setItem("token", token); // ✅ Save token
            navigate("/dashboard"); // Navigate to Dashboard to make the user see their content
        } catch (err: any) {
            console.error(err);
